...
exports.postData = function(req, res) {
  ...
  var data;
  var key='?key=';
  var action = '/forms/postData';

  if(req.query.key){
    key += req.query.key;
    p = logData.getAll(req.query.key).then(JSON.stringify);
  } else {
    key = '';
    p = readFile('postData.txt','utf8');
  }

  p.then(function(d){
    data = d ||'File does not exist';
  }).catch(function(){
    data = 'File does not exist';
    key= '';
  }).finally(function(){
    res.render(VIEWS_DIR + 'postData', { postData: data, action: action+key });
